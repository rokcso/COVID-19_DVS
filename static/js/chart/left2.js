...
function start_roll() {
    ...
    var li1 = document.getElementById('risk_wrapper_li1');
    var li2 = document.getElementById('risk_wrapper_li2');
    li2.innerHTML = li1.innerHTML;

    function runTable() {
        // 如果 tr 到了表格的顶部
        if (li2.offsetHeight - wrapper.scrollTop <= 0) {
            wrapper.scrollTop -= li1.offsetHeight;
        } else {
            wrapper.scrollTop++;
        }
    }
    var runTime = setInterval(runTable, speed);
    wrapper.onmouseover = function() {
        clearInterval(runTime);
    };
    wrapper.onmouseout = function() {
        runTime = setInterval(runTable, speed);
    }
}